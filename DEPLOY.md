...
- `third-molar/sw.js` → `const CACHE = "tma-v1-4-16";`
- `sedation/sw.js` → `var CACHE = "sedation-v3";`

**Keep the prefixes.** Both tools share one origin, and caches are shared
per-origin rather than per folder. Each worker only deletes caches carrying its
own prefix (`tma-` / `sedation-`), so they do not destroy each other. A rename
that drops the prefix would break that.
