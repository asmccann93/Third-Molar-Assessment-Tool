...
self.addEventListener("activate", (e) => {
  e.waitUntil(
    caches
      ...
      // sedation-*, la-* and asa-* on every deploy. The trailing hyphen matters:
      // "tm-" must not match the hub's "tma-" caches, and vice versa.
      .then((keys) =>
        Promise.all(
          keys.filter((k) => k.indexOf("tm-") === 0 && k !== CACHE).map((k) => caches.delete(k))
        )
      )
