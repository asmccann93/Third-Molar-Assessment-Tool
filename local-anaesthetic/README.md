...
This folder goes **inside** the site repository, not at its root. The three tools
share one origin, and the third molar tool owns the root — it has an `index.html`,
`sw.js`, `manifest.webmanifest` and icons with exactly the same filenames as these,
so unpacking this at the root overwrites it.

```
<site repo>/
├── index.html            third molar tool — do not overwrite
├── sw.js
├── sedation/
└── local-anaesthetic/    this folder
```

The folder name must stay `local-anaesthetic`, because the switcher links in
`index.html` point at `/local-anaesthetic/`. Renaming it means changing those hrefs
here and in the other two tools' switcher blocks.

Nothing is compiled and nothing is minified — `index.html` is edited directly.
